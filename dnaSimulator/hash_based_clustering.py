...
class HashBasedCluster:
    ...
    def hash_based_cluster(self, number_of_steps, windows_size, similarity_threshold, report_func, clustering_input, clustering_output):
        reads_err = []
        strand_id = 0
        ...
        #temp_evyat_path = clustering_result_dir

        #temp_evyat_path += 'temp.txt'
        with open(clustering_input, 'r') as evyat_f:
            line = "j"
            while line:
        ...
        for i in range(0, len(reads_err)):
            if i % 100 == 0:
                report_func(total_bar_size, 1 + int((tmp_bar * i) / len(reads_err)), False)
        ...
        for i in range(1, len(reads_err)):
            if i % 100 == 0:
                report_func(total_bar_size, tmp_bar + int((tmp_bar * i) / len(reads_err)), False)
        ...
        for lcl_step in range(0, local_comm):
            report_func(total_bar_size, (2 * tmp_bar) + lcl_step, False)
        ...
        with open(clustering_output, 'w', newline='\n') as temp_f:
