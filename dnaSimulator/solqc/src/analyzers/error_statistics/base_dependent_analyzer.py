...
class BaseDependentAnalyzer(Analyzer):
                        ...
                        if not(idx-1>=0 and str_cigar[idx-1]=='1'):
                            ...
                            if(idx_var<len(str_variant)):
                                ...
                                if (idx_var < len(read_seq)):
                                    ...
                                    idx_var+=1
                    elif idx +1 < len(str_cigar) and str_cigar[idx+1]==DELETION:
    ...
    def generate_content(self):
        ...
        headline = Content(Content.Type.TEXT, "Error rates, stratified by symbol")
    ...
    def heat_map_plot(self):
        ...
        for i in range(len(bases)):
            for j in range(len(errors)):
                ext = ax.text(i, j, round(rates[j, i], 3),
                              ha="center", va="center", color=colors[j], fontsize=14)
        cbar = ax.figure.colorbar(im, ax=ax, **{})
        cbar.ax.set_ylabel("Error rates in percent", rotation=-90, va="bottom", fontsize=28)
