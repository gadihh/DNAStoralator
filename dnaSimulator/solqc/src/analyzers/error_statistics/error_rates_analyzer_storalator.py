...
class ErrorRatesAnalyzer_storalator(Analyzer):
    ...
    def plot_error_probabilities(self, plot=True):
        labels = ["substitution", "insertion", "one_base_deletion", "long_deletion", "total_del"]
        probs = np.zeros(5).astype(float)
        probs[0] = max(self.substitution,0.000000001)
        probs[1] = max(self.insertion,0.000000001)
        probs[2] = max(self.base_deletion,0.000000001)
        probs[3] = max(self.long_deletion,0.000000001)
        probs[4] = max(self.total_deletion,0.00000001)
        '''
        bar_width = 0.40
        ...
        '''
        stats_dict ={} #key "statistics" 
        
        for i, p in enumerate(probs.tolist()):
            stats_dict[labels[i]]=p/100
